...
import 'package:pasal/src/features/orders/presentation/my_orders_screen.dart';
...
class ProfileScreen extends ConsumerWidget {
  ...
  void _navigateToOrders(BuildContext context, {String? statusFilter}) {
    Navigator.of(context).push(
      MaterialPageRoute(
          builder: (context) => MyOrdersScreen(statusFilter: statusFilter)),
    );
  }
          ...
          child: Column(
            ...
            children: [
              ...
              SingleChildScrollView(
                ...
                child: Row(
                  children: [
                    _buildOrderItem(context, Icons.pending_actions_outlined,
                        'Pending', () => _navigateToOrders(context, statusFilter: 'Pending')),
                    const SizedBox(width: 16),
                    _buildOrderItem(context, Icons.local_shipping_outlined,
                        'Shipped', () => _navigateToOrders(context, statusFilter: 'Shipped')),
                    const SizedBox(width: 16),
                    _buildOrderItem(context, Icons.rate_review_outlined, 'Review',
                        () => _navigateToOrders(context, statusFilter: 'Review')),
                    const SizedBox(width: 16),
                    _buildOrderItem(
                        context, Icons.all_inbox_outlined, 'Pre-order', () {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                          content: Text('Pre-order functionality is not built yet.'),
                          duration: Duration(seconds: 2),
                        ),
                      );
                    }),
                    const SizedBox(width: 16),
                    _buildOrderItem(context, Icons.check_circle_outline,
                        'Delivered', () => _navigateToOrders(context, statusFilter: 'Delivered')),
                  ],
                ),
              ),
            ],
  ...
  Widget _buildStatItem(
      BuildContext context, String title, String count, VoidCallback onTap) {
    final isDarkMode = Theme.of(context).brightness == Brightness.dark;
    final titleColor =
        isDarkMode ? AppColors.darkOnSurface : AppColors.lightOnSurface;
